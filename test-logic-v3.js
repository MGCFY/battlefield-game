/* 战场沙盘 v3 · 规则断言测试
   直接从 battlefield-v3.html 抽取真实脚本，在 Node 里用最小 DOM 桩运行，
...
globalThis.__T = {
  ...
  fn: { newGame, renderDeploy, confirmDeploy, startPlay, render, endTurn,
        ...
        deploySpent, spawnFromConfig, resolveMissiles, hasGround, markSeen, grid, key, manh,
        strikeCells, viewLocked, boardMove, beginMoveMode,
...
/* ============ 4. 迷雾可见性 ============ */
{
  ...
  const foeIn = mk(1, "inf", 100, 11, 11);      // 曼哈顿2 → 菱形视野内可见
  const foeOut = mk(1, "inf", 100, 15, 15);     // 距离10 → 不可见
  const foeAir = mk(1, "recon", 1, 10, 12);     // 曼哈顿2，但空中 → 永不可见
  const v = vis(0);
...
/* ============ 10. 菱形视野与盲打（v3 新语义） ============ */
{
  ...
  const v = vis(0);
  ok(v.has("10,12"), "视野：菱形 3 格内可见（曼哈顿2）");
  ok(v.has("11,12"), "视野：菱形边界（曼哈顿3）可见");
  ok(!v.has("12,12"), "视野：斜角 (12,12) 曼哈顿4，菱形视野外（旧方阵视野为可见）");
  const scout = mk(0, "scout", 1, 10, 10);     // 视野6 拉动全方视野
  const v2 = vis(0);
  ok(v2.has("10,15"), "视野：侦察兵把全方视野拉到菱形 6 格");
  ok(v2.has("13,13"), "视野：(13,13) 距侦察兵曼哈顿6，恰好可见");
  ok(!v2.has("13,14"), "视野：(13,14) 曼哈顿7，不可见");
  // v3 盲打：打击目标只看射程（曼哈顿菱形），不看视野
  const cells = F.strikeCells(art);
  ok(cells.has("10,14"), "盲打：射程4内正前方格可直接打击（无需视野）");
  ok(cells.has("12,12"), "盲打：曼哈顿4 的斜角格可打击（切比雪夫仅为2，旧方阵会漏）");
  ok(cells.has("11,13"), "盲打：菱形射程边界（曼哈顿4）可打击");
  ok(!cells.has("12,13"), "盲打：曼哈顿5 超出射程，不可打击");
  ok(!cells.has("10,10"), "盲打：不能打击自身所在格");
}
...
{
  // A. 视角锁定
  resetMap(); T.turnNo = 1; T.phase = "play"; T.current = 0; T.viewMode = "auto";
  eq(F.viewLocked(), false, "视角：自动视角不锁定");
  T.viewMode = "god";  eq(F.viewLocked(), true,  "视角：上帝视角锁定操作");
  T.viewMode = "red";  eq(F.viewLocked(), false, "视角：当前红方时看红方视角不锁定");
  T.current = 1;       eq(F.viewLocked(), true,  "视角：轮到蓝方时看红方视角锁定");
  T.viewMode = "blue"; eq(F.viewLocked(), false, "视角：轮到蓝方时看蓝方视角不锁定");

  // B. 侦察兵奔袭 3 格二次确认
  resetMap(); T.phase = "play"; T.current = 0; T.viewMode = "auto";
  T.nextId = 1; T.corps = [];
  const sc = mk(0, "scout", 5, 10, 10);
  T.selId = sc.id; T.acted.clear();
  const vis = F.computeVision(0);
  F.beginMoveMode(sc, 3);
  eq(T.mode, "move", "奔袭：进入移动模式");
  F.boardMove(10, 13);                       // 第一次点 3 格外目标
  eq(sc.r === 10 && sc.c === 10, true, "奔袭：第一次点击不移动，仅提示确认");
  ok(T.logEntries.some(e => e.msg.includes("奔袭3格")), "奔袭：给出二次确认提示");
  F.boardMove(10, 12);                       // 改点 2 格目标 → 无需确认，直接移动
  eq(sc.r === 10 && sc.c === 12, true, "奔袭：2 格路径无需二次确认");
  sc.r = 10; sc.c = 10; sc.fatigue = 0; T.acted.clear();
  F.beginMoveMode(sc, 3);
  F.boardMove(10, 13);                       // 3 格路径 → 第一次需确认
  eq(sc.r === 10 && sc.c === 10, true, "奔袭：第一次点击不移动");
  F.boardMove(13, 10);                       // 改点另一条 3 格路径 → 应重新要求确认
  eq(sc.r === 10 && sc.c === 10, true, "奔袭：更换 3 格目标后需重新确认");
  F.boardMove(13, 10);                       // 同路径第二次 → 执行
  eq(sc.r === 13 && sc.c === 10, true, "奔袭：同路径第二次确认后移动成功");
  ok(sc.fatigue > 0, "奔袭：3 格后下回合疲劳（不能移动）");
}
